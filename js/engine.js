...
take = function(item) {
    if (data.player.currentLocation.objects = removeItem(item, data.player.currentLocation.pockets)) {
        data.player.pockets = addItem(item, data.player.pockets)
        return true
    }
    return false
}
...
drop = function(item) {
    if (data.player.pockets = removeItem(item, data.player.pockets)) {
        data.player.currentLocation.objects = addItem(item, data.player.currentLocation.objects)
        return true
    }
    return false
}
...
removeItem = function(string, list) {
        if (list[string] != undefined && list[string] > 0) {
            list[string]--
                if (list[string] == 0) {
                    delete list[string]
                }
...
listObjects = function(place) {
    var toReturn = ""
    if (Object.keys(place.objects).length > 0) {
        toReturn += "you see: </br>"
        for (var item in place.objects) {
            console.log(item)
            toReturn += item + ": (" + place.objects[item] + ") </br>"
        }
    } else {
...
$(document).ready(function() {
    ...
    $("form").submit(function() {
        ...
        if (input.indexOf("help") > -1) {
            ...
            input = trimInput(input, "walk to")
            walkTo(input)
            println(data.messages.moveMessage + input)

            //take items
        } else if (input.indexOf("take") > -1) {
            input = trimInput(input, "take")
            if (take(input)) {
                println(data.messages.pickUpSuccess + addArticle(input))
            }else{
            	println(data.messages.pickUpError + addArticle(input))
            }
            //drop items
        } else if (input.indexOf("drop") > -1) {
            input = trimInput(input, "drop")
            if (drop(input)) {
                println(data.messages.dropText + addArticle(input))
            }
